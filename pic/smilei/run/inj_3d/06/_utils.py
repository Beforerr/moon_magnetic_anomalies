...
class SmileiPostProcessing(object):
    ...
    def __init__(self, results_path="."):
        ...
        self.SmileiSimulation = happi.Open(
            results_path=results_path, verbose=False, show=True,reference_angular_frequency_SI=w_r.value
        )
    ...
    def plot_scalar(self):
        ...
        S = self.SmileiSimulation
        units = ['s']
        Utot = S.Scalar("Utot", label="Total energy",units=units)
        Ukin = S.Scalar("Ukin", label="Total kinetic energy",units=units)
        Uelm = S.Scalar("Uelm", label="Total electromagnetic energy",units=units)
        Ubal = S.Scalar("Ubal", label="Balance energy",units=units)
        Ukin_ = {}
        for species in S.namelist.Species:
            if not species.is_test:
                Ukin_[species.name] = S.Scalar(
                    "Ukin_" + species.name, label="Total " + species.name + " energy",
                units=units)

        _bak = mpl.get_backend()
        mpl.use('Agg')
        happi.multiPlot(
            Utot,
            Ukin,
            ...
            Ubal,
            *Ukin_.values(),
            saveAs="figures/scalars.png",
        )
        plt.savefig("figures/scalars.png")
        mpl.use(_bak)
