...
Main(
    ...
    simulation_time=simulation_time_norm ,
...
for species in solar_species:
    ...
    Species(
        ...
        position_initialization=position_initialization,
        momentum_initialization=momentum_initialization,
        particles_per_cell=test_particles_per_cell,
        mass=species.mass_norm,
        charge=species.charge_norm,
        number_density=0,
        boundary_conditions=boundary_conditions,
...
ParticleInjector(
    name="injector_solar_electron_test",
    species="solar_electron_test",
    box_side="xmax",
    number_density=n_solar_norm,
)
ParticleInjector(
    name="injector_solar_proton_test",
    species="solar_proton_test",
    box_side="xmax",
    position_initialization="injector_solar_electron_test",
    number_density=n_solar_norm,
)
...
if _DiagTrackParticles:
    def my_filter(particles):
        _len = len(particles.id)
        _subset_ratio = 1e-3/_len
        random_subset = np.random.random(_len) < _subset_ratio
        if dimensions == 3:
            threshold = True
        if dimensions == 2:
            threshold = True
