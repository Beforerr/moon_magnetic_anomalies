...
def LMA_field_np():
    ...
    x, y, z = domain(grid_length, cell_length, number_of_cells)
    ...
    Bx = mu_0 / (4 * pi)*(3*(r_x-r_0_x)*(mr_dot)/r**5 - m_d_x/r**3)/B_r
    By = mu_0 / (4 * pi)*(3*(r_y-r_0_y)*(mr_dot)/r**5 - m_d_y/r**3)/B_r
    Bz = mu_0 / (4 * pi)*(3*(r_z-r_0_z)*(mr_dot)/r**5 - m_d_z/r**3)/B_r
    return Bx, By, Bz
