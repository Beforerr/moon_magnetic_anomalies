...
if dimensions == 2:
    grid_length = [L_x, L_z]
    number_of_cells = [_ * space_ratio for _ in [80, 64]]
if dimensions == 3:
    grid_length = [L_x, L_y, L_z]
    number_of_cells = [_ * space_ratio for _ in [80, 64, 64]]
...
r_zero = ((mu0 * M_d / 4 / np.pi / B_IMF) ** (1 / 3)).si

def magnetic_field(r_x, r_y, r_z):
    from plasmapy.formulary import MagneticDipole
