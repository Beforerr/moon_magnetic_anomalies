...
By_expression=  "B_IMF + mu0 / 4 / pi * ( 3 * (y-p0_y) * ((x - p0_x)*m_d_x + (y - p0_y)*m_d_y +  (z - p0_z)*m_d_z )/ ((x-p0_x)**2+(y- p0_y)**2+(z-p0_z)**2)**(5/2) - m_d_y/((x-p0_x)**2+(y- p0_y)**2+(z-p0_z)**2)**(3/2))"
Bz_expression=  "mu0 / 4 / pi * ( 3 * (z-p0_z) * ((x - p0_x)*m_d_x + (y - p0_y)*m_d_y +  (z - p0_z)*m_d_z )/ ((x-p0_x)**2+(y- p0_y)**2+(z-p0_z)**2)**(5/2) - m_d_z/((x-p0_x)**2+(y- p0_y)**2+(z-p0_z)**2)**(3/2))"

B_dipolar = picmi.AnalyticAppliedField(Bx_expression=Bx_expression,By_expression=By_expression,Bz_expression=Bz_expression, lower_bound=[xmin, ymin, zmin],upper_bound=[xmax,ymax,zmax], p0_x = s_d.si.value, p0_y = 0. , p0_z = 0. , m_d_x = 0.,m_d_y = M_d_value,m_d_z = 0,B_IMF=B_IMF.si.value)
...
part_diag1 = picmi.ParticleDiagnostic(
    period=diagnostic_intervals, species=[electrons, protons], name="diag1", data_list=['position','momentum','weighting'],
)
