...
if geometry == "2Dcartesian":
    ...
    grid.add_refined_region(level=1, lo=[xmin, zmin / 2], hi=[(s_d + 1.2*L).si.value, zmax / 2])

    # grid.add_refined_region(level = 2,
    #                     lo = [xmin, zmin/2],
    #                     hi = [xmax/3/4, zmax/2])
...
part_diag1 = picmi.ParticleDiagnostic(
    ...
    warpx_uniform_stride = 64,
)

# diagnostic_intervals = "::{}".format(int(max_steps / 1000))
# field_diag2 = picmi.FieldDiagnostic(
#     name="diag2",
#     grid=grid,
#     period=diagnostic_intervals,
#     data_list=["rho", "rho_electron", "rho_proton", "E", "B", "J"],
#     lower_bound=[(s_d + L).si.value, 0, 0],
#     upper_bound=[(s_d + L).si.value, 0, 0],
